...
// Extension for array of telemetry data
extension Array where Element == TelemetryData {
    // Get maximum speed from array of telemetry data
    func maxSpeed() -> Double {
        return self.map { $0.speed }.max() ?? 0
    }
    ...
    // Get max G-force
    func maxGForce() -> Double {
        return self.map { $0.getTotalGForce() }.max() ?? 0
    }
    
    // Get max lateral G-force
    func maxLateralGForce() -> Double {
        return self.map { abs($0.getLateralGForce()) }.max() ?? 0
    }
    
    // Get max longitudinal G-force
    func maxLongitudinalGForce() -> Double {
        return self.map { abs($0.getLongitudinalGForce()) }.max() ?? 0
    }
    ...
    func totalDistance() -> Double {
        ...
        for i in 0..<(self.count - 1) {
            totalDistance += self[i].distance(to: self[i+1])
        }
